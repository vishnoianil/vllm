...
This kernel targets the NVIDIA Hopper GPU and uses:
  ...
  - Per-tensor scaling applied in the epilogue (Int32 -> Float32 -> scale -> output)
...
Run self-test:
    python vllm/kernels/cutedsl/scaled_mm_sm90_int8.py
"""
...
class ScaledMmSm90Int8Kernel:
    ...
    def kernel(
        ...
        for epi_idx in cutlass.range_constexpr(epi_tile_num):
            ...
            scaled_vec = float_vec * combined_scale
...
def _create_tensors(m, n, k):
    """Create random INT8 inputs, FP32 scales, and BF16 output tensor."""
    import torch
...
def _compute_reference(a_cpu, b_cpu, scale_a_cpu, scale_b_cpu, c_dtype):
    ...
    INT8 inputs are cast to FP32 for the reference matmul.
    """
    import torch
    import cutlass.torch as cutlass_torch

    # einsum "mkl,nkl->mnl" in float
...
def run_test(
    ...
    A wider tolerance is used because INT8 GEMM with INT32 accumulation
    followed by FP32 scaling and BF16 truncation can introduce rounding
    differences compared to a pure-FP32 reference.
    """
    import torch
...
def main():
    ...
    test_cases = [
        ...
        ((256, 256, 256), (128, 128), (2, 1)),   # default config
        ((128, 128, 256), (64, 128), (2, 1)),     # M128 config
        ((64, 64, 256), (64, 64), (1, 1)),        # M64 config
    ]
