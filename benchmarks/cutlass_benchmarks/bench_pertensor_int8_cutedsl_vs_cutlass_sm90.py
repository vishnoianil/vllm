"""
Benchmark: CuTe DSL kernel vs CUTLASS C++ kernel for per-tensor INT8 scaled GEMM
on NVIDIA Hopper (SM90).

Compares:
  1. CuTe DSL  - vllm/kernels/cutedsl/scaled_mm_sm90_int8.py
...
Both kernels compute:  D = scale_a * scale_b * (A @ B^T)
  ...
  - B: (N, K) INT8, col-major  (stored as (N, K).T for CUTLASS C++)
  - D: (M, N) BF16 or FP16
  - scale_a, scale_b: per-tensor FP32 scalars
  - Accumulator: INT32, converted to FP32 before scaling

Usage:
    python benchmarks/cutlass_benchmarks/bench_pertensor_int8_cutedsl_vs_cutlass_sm90.py
...
def make_tensors_cutlass(
    ...
    """Create INT8 A/B and per-tensor FP32 scales for the CUTLASS C++ kernel.

    CUTLASS expects:
      a: (M, K) int8, row-major (contiguous)
      b: (N, K).T -> (K, N) int8, column-major
...
def make_tensors_cutedsl(m: int, n: int, k: int, c_dtype_str: str):
    """Create CuTe-style 3D tensors for the CuTe DSL INT8 kernel.

    A: (M, K, 1) K-major, B: (N, K, 1) K-major, C: (M, N, 1) N-major.
    scale_a, scale_b: (1, 1, 1) scalar tensors.
    """
    ...
    sfa_cpu = cutlass_torch.matrix(1, 1, 1, False, scale_dtype)
...
class CuteDslBenchWrapper:
    """Wraps the CuTe DSL ScaledMmSm90Int8Kernel for benchmarking.
    ...
    def __init__(
        ...
        # Compile once
        print(
            f"      [CuTe DSL] Compiling INT8 kernel "
        ...
        for n in n_sizes:
            for k in k_sizes:
                ...
                if _cute_dsl_available:
                    ...
                    if auto_tile:
                        from scaled_mm_sm90_int8 import select_tile_config
                ...
                # ---- PyTorch INT8 matmul baseline ----
                try:
                    a_pt = torch.randint(
                        -128, 127, (m, k), device="cuda", dtype=torch.int8
                    )
                    b_pt = torch.randint(
                        -128, 127, (n, k), device="cuda", dtype=torch.int8
                    )

                    def pytorch_fn(_a=a_pt, _b=b_pt, _dt=out_dtype):
                        torch.mm(
                            _a.to(dtype=torch.bfloat16),
                            _b.to(dtype=torch.bfloat16).t(),
                        )
                    ...
                    tflops = flops / (med * 1e-3) / 1e12
                    results.append(
                        ("PyTorch mm (bf16 cast)", med, mean, mn, tflops)
                    )
                except Exception as e:
                    results.append(
                        (f"PyTorch mm [ERR: {e}]", -1, -1, -1, 0)
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Benchmark CuTe DSL vs CUTLASS C++ per-tensor INT8 scaled GEMM "
            "on NVIDIA Hopper (SM90)"
        ),
    ...
    print()
    print("=" * 70)
    print("  Per-Tensor INT8 Scaled GEMM Benchmark: CuTe DSL vs CUTLASS C++")
