...
def cutlass_scaled_mm(
    ...
    if envs.VLLM_SWITCH_CUTILE:
        ...
        torch.ops.vllm.cutile_scaled_mm_out(out_cutile, a, b, scale_a, scale_b, bias)
        ...
        if not torch.compiler.is_compiling():
            ...
            if not torch.allclose(out_cutile, out_cutlass, rtol=1e-2, atol=1e-2):
                max_diff = (out_cutile - out_cutlass).abs().max()
                print(
                    f"Max difference between cutile and reference implementation: {max_diff.item()}"
                )
                raise ValueError(
