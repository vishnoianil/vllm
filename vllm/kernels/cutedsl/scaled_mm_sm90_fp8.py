...
Computes: D[m,n] = scale_a[m] * scale_b * (A @ B^T)[m,n]
  - A is MxK  (FP8 e4m3, row-major / K-major)
  - B is NxK  (FP8 e4m3, col-major / K-major)
  - D is MxN  (BF16 or FP16, row-major / N-major)
  - scale_a is per-token FP32 (Mx1) or per-tensor (scalar)
  - scale_b is per-tensor FP32 scalar
...
This kernel targets the NVIDIA Hopper GPU and uses:
  ...
  - Per-token/per-tensor scaling fused in the epilogue via column broadcast
...
class ScaledMmSm90Fp8Kernel:
    ...
    def __call__(
        ...
        :param scale_a: Per-token scale for A (Mx1xL FP32).
        :param scale_b: Per-tensor scale for B (1x1xL FP32 scalar).
    ...
    def kernel(
        ...
        # Partition broadcast scale_a (M,N,L) like C for per-element scaling.
        # The N-stride is 0, so all columns in the same row read the same
        # scale_a[m] value — this is per-token scaling via column broadcast.
        gSA_mnl = cute.local_tile(
            gScaleA, self.tile_shape_mnk, tile_coord_mnkl, proj=(1, 1, None)
        )
        ...
        # =====================================================================
        #  Load scalar scale_b from global memory
        # =====================================================================
        ...
        # =====================================================================
        #  EPILOGUE: pre-scale accumulators and write back
        # =====================================================================
        ...
        # Pre-scale FP32 accumulators with per-token scale_a and scalar
        # scale_b.  tCgSA[i] reads scale_a[row] from global memory via the
        # broadcast layout (stride-0 on N); L1 cache handles reuse across
        # columns.  This fuses the scaling into the epilogue, eliminating
        # the M×N FP32 intermediate that post-kernel scaling would require.
        acc_count = cute.size(accumulators)
        for acc_i in cutlass.range(acc_count, unroll=1):
            accumulators[acc_i] = accumulators[acc_i] * tCgSA[acc_i] * scale_b_val
        ...
        for epi_idx in cutlass.range_constexpr(epi_tile_num):
            # Copy pre-scaled accumulator fragment to D registers
            for epi_v in cutlass.range_constexpr(size_tRS_rD):
                tRS_rD[epi_v] = tRS_rAcc[epi_idx * size_tRS_rD + epi_v]

            # Convert to output dtype (accumulators already scaled above)
            acc_vec = tRS_rD.load()
...
def _create_tensors(m, n, k):
    ...
    # scale_a: per-token (M, 1, 1), scale_b: scalar (1, 1, 1)
    scale_a_cpu = cutlass_torch.matrix(1, m, 1, False, scale_dtype)
...
def _compute_reference(a_cpu, b_cpu, scale_a_cpu, scale_b_cpu, c_dtype):
    """Reference: D[m,n] = scale_a[m] * scale_b * (A @ B^T)[m,n], in FP32."""
    import torch
    import cutlass.torch as cutlass_torch

    # einsum "mkl,nkl->mnl"
    ref = torch.einsum("mkl,nkl->mnl", a_cpu.float(), b_cpu.float())
    # scale_a is per-token (M, 1, 1) — broadcast across N
    sa = scale_a_cpu.float().reshape(-1, 1, 1)  # (M, 1, 1)
